...
/*   Updated: 2023/11/06 23:03:32 by gdurmaz          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main(int argc, char **argv, char **envp)
{
    ...
    if (argc == 5)
    {
        ...
        if (pid == 0) // child process
            child_process(argv, envp, end); // writes from input
        waitpid(pid, NULL, 0); // wait till child p. finished
        parent_process(argv, envp, end); // reads from child then writes
    }
    else
